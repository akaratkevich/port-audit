...
/*
Load two Excel sheets for comparison: a baseline and a newly generated sheet named with a current timestamp.

It reads data from both sheets, compares them, and logs the number of differences found.

Parameters:
  filename string - the file path of the Excel workbook to open.

Returns:
  error - returns an error if any step of the process fails, such as opening the file, reading data, or if sheets are missing.
*/
...
func CompareExcelSheets(filename string, logger *pterm.Logger) error {
	...
	diffCount := compareData(refData, newData) // Compare data from the two sheets
...
Generate reports for each node detailing any changes or discrepancies found between the data sets.

Parameters:
  refData, newData []InterfaceData - slices containing the data from the reference and new sheets, respectively.

Returns:
  int - the number of differences found between the reference and new data.
*/
...
func dataEquals(a, b InterfaceData) bool {
	return a.Type == b.Type && a.Description == b.Description && a.Status == b.Status &&
		a.Speed == b.Speed && a.Duplex == b.Duplex && a.VLAN == b.VLAN
}
